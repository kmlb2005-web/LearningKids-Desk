...
namespace MathAdminApp.LogicaNegocio
{
    public class ProyectoBLL
    {
        ...
        public List<Proyecto> ObtenerPorDocente(int idDocente)
        {
            if (idDocente <= 0)
                return new List<Proyecto>();

            return _dal.ObtenerPorDocente(idDocente);
        }

        public List<Proyecto> ObtenerVisibles(Usuario usuarioActual)
        {
            return usuarioActual.IdRol == 1
                ? ObtenerTodos()
                : ObtenerPorDocente(usuarioActual.IdUsuario);
        }
