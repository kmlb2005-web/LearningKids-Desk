...
namespace MathAdminApp.LogicaNegocio
{
    ...
    public class UsuarioBLL
    {
        ...
        {
            if (idDocente <= 0)
                throw new ArgumentException("ID de docente invalido.");

            return _dao.ObtenerAlumnosPorDocente(idDocente);
        }
        ...
        public List<Usuario> ObtenerUsuariosVisibles(Usuario usuarioActual)
        {
            return usuarioActual.IdRol == 1
                ? ObtenerUsuarios()
                : ObtenerAlumnosPorDocente(usuarioActual.IdUsuario);
        }
        ...
        public bool AgregarAlumnoParaDocente(Usuario usuario, int idDocente)
        {
            usuario.IdRol = 3;

            bool agregado = AgregarUsuario(usuario);

            if (!agregado || usuario.IdUsuario <= 0)
                return false;

            DocenteAlumnoBLL docenteAlumnoBLL = new();
            return docenteAlumnoBLL.Agregar(new DocenteAlumno
            {
                IdDocente = idDocente,
